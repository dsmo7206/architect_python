...
layout (location=0) in vec3 in_position_WS[3];
layout (location=1) in vec3 in_ambient[3];
layout (location=2) in vec3 in_diffuse[3];
layout (location=3) in vec4 in_specular[3];
...
void main()
{
    // This gets applied to each vertex
    out_normal_WS = normalize(
        cross(
            in_position_WS[1] - in_position_WS[0],
            in_position_WS[2] - in_position_WS[0]
        )
    );
    out_ambient = in_ambient[0];
    out_diffuse = in_diffuse[0];
    out_specular = in_specular[0];

    out_position_WS = in_position_WS[0];
    gl_Position = gl_in[0].gl_Position;
    EmitVertex();

    out_position_WS = in_position_WS[1];
    gl_Position = gl_in[1].gl_Position;
    EmitVertex();

    out_position_WS = in_position_WS[2];
...
class BlinnWithoutNormalsProgram(SingletonProgram):
    def __init__(self):
        vs = common.compileShaderStage(vs_source, GL.GL_VERTEX_SHADER)
        gs = common.compileShaderStage(gs_source, GL.GL_GEOMETRY_SHADER)
        fs = common.compileShaderStage(fs_source, GL.GL_FRAGMENT_SHADER)

        self.program = common.compileShaderProgram(vs, gs, fs)
